...
OpenRouter provides a unified API for multiple LLM providers using
...
Example:
    ...
    >>> client = provider.model("anthropic/claude-sonnet-4")
...
class OpenRouterModelClient:
    """Model-specific client for OpenRouter.

    Created via OpenRouterProvider.model() - do not instantiate directly.
    """
    ...
    def _parse_response(self, response: Any) -> Message:
        """Convert OpenRouter response to nous Message."""
...
class OpenRouterProvider:
    ...
    OpenAI-compatible API. Models are prefixed with their provider
    ...
    def model(self, model_id: str) -> OpenRouterModelClient:
        ...
        Args:
            model_id: The model ID (e.g., "anthropic/claude-sonnet-4", "openai/gpt-4o").

        Returns:
            An OpenRouterModelClient for the specified model.
        """
